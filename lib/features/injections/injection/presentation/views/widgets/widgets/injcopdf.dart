...
class Injworkercopdf {
  static Future<File> generatepdf({
    required BuildContext context,
    required double totalval,
    required Uint8List imageBytes,
    required List<Workercomodel> categories,
  }) async {
    ...
    final data = categories.map((item) {
      return [
        "${((BlocProvider.of<AverageCubit>(context).averagerateprice[item.job]! * item.quantity) / 100).toStringAsFixed(1)}",
        BlocProvider.of<AverageCubit>(context).averagerateprice[item.job],
        "${((item.quantity / ((item.time * 60) / BlocProvider.of<AverageCubit>(context).averagerate[item.job])) * 100).round()} %",
        ((item.time * 60) /
                BlocProvider.of<AverageCubit>(context).averagerate[item.job])
            .round(),
        item.quantity,
        item.time,
        item.job,
      ...
      build: (context) => [
        ...
        pw.Container(
            ...
            child: pw.Row(
                ...
                children: [
                  pw.Text("تجميع ${categories[0].name}",
                      style: pw.TextStyle(
                          fontSize: 20,
        ...
        pw.Table.fromTextArray(
            ...
            columnWidths: {
              ...
              2: pw.FixedColumnWidth(PdfPageFormat.cm * 2),
              3: pw.FixedColumnWidth(PdfPageFormat.cm * 2),
              4: pw.FixedColumnWidth(PdfPageFormat.cm * 2),
              5: pw.FixedColumnWidth(PdfPageFormat.cm * 2),
              6: pw.FixedColumnWidth(PdfPageFormat.cm * 2),
              7: pw.FixedColumnWidth(PdfPageFormat.cm * 2),
            },
        ...
        pw.Container(
            ...
            child: pw.Row(children: [
              ...
              pw.Expanded(
                child: pw.Text(
                    intr.NumberFormat("#,##0.##").format(totalval / 100),
                    textAlign: pw.TextAlign.center,
                    style: pw.TextStyle(color: PdfColors.blue800)),
              )
            ]),
    ...
    return await savepdf("تجميع  ${categories[0].name}", pdf);
  }
